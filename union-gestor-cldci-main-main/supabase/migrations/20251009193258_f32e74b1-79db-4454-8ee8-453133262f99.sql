...
-- Fecha: 2025-10-09
-- Objetivo: Proteger datos sensibles con enmascaramiento
-- Tablas: miembros, seccionales, delivery_orders
-- =====================================================

-- ============================================
-- PASO 1: Crear funciones de enmascaramiento
-- ============================================

-- Función para enmascarar email (muestra solo primeros 3 caracteres)
...
-- Función para enmascarar teléfono (muestra solo últimos 4 dígitos)
...
-- Función para enmascarar cédula (muestra solo últimos 3 dígitos)
...
-- Función para enmascarar dirección
...
-- ============================================
-- PASO 2: TABLA MIEMBROS - Nuevas políticas
-- ============================================

-- Eliminar políticas existentes que exponen datos
DROP POLICY IF EXISTS "moderator_restricted_access" ON public.miembros;

-- Nueva política: moderadores ven miembros de su org CON ENMASCARAMIENTO
...
-- Crear vista segura para miembros con datos enmascarados
...
SELECT 
  ...
  m.updated_at,
  m.observaciones,
  -- Campos enmascarados según rol del usuario
  CASE 
...
-- ============================================
-- PASO 3: TABLA SECCIONALES - Corrección
-- ============================================

-- Eliminar política que expone contactos
DROP POLICY IF EXISTS "Users can view seccionales of their organization" ON public.seccionales;

-- Nueva política restrictiva: solo info básica
...
-- Crear vista segura para seccionales
...
SELECT 
  ...
  s.created_at,
  s.updated_at,
  -- Campos de contacto enmascarados
  CASE 
    ...
    WHEN EXISTS (
      ...
      WHERE m.id = s.coordinador_id AND m.user_id = auth.uid()
    ) THEN s.email
...
WHERE 
  EXISTS (
    SELECT 1 FROM user_roles ur 
    WHERE ur.user_id = auth.uid() 
    AND (ur.organizacion_id = s.organizacion_id OR ur.role = 'admin'::app_role)
  );

-- ============================================
-- PASO 4: TABLA DELIVERY_ORDERS - Restricción
-- ============================================

-- Eliminar política que expone todos los pedidos
DROP POLICY IF EXISTS "Company users can view their orders" ON public.delivery_orders;

-- Nueva política: solo pedidos asignados
...
-- Vista segura para delivery orders
...
SELECT 
  ...
  ord.pickup_lat,
  ord.pickup_lng,
  ord.preferred_time_start,
  ord.preferred_time_end,
  ord.weight,
  ord.volume,
  ord.requires_signature,
  ord.requires_pin,
  ord.pin_code,
  ord.tracking_token,
  -- Datos sensibles solo visibles cuando está en tránsito/entregado
  CASE 
...
WHERE 
  ...
  OR EXISTS (
    ...
    FROM drivers d
    JOIN user_roles ur ON ur.user_id = d.user_id
    WHERE d.company_id = ord.company_id
    AND ur.user_id = auth.uid()
    AND ur.role IN ('admin'::app_role, 'moderador'::app_role)
  );
...
CREATE INDEX IF NOT EXISTS idx_data_access_audit_accessed_at ON public.data_access_audit(accessed_at);

-- ============================================
-- PASO 7: Documentación
-- ============================================

COMMENT ON VIEW public.miembros_seguros IS 
'Vista segura con enmascaramiento automático de datos sensibles según rol. 
Admins: acceso completo. Moderadores: datos enmascarados. Usuarios: solo su registro.';

COMMENT ON VIEW public.seccionales_seguras IS 
'Vista segura con contactos enmascarados. Solo admins, moderadores y coordinadores ven contactos completos.';

COMMENT ON VIEW public.delivery_orders_seguras IS 
'Vista segura con direcciones ocultas hasta que el pedido esté en tránsito o entregado.';

COMMENT ON FUNCTION public.mask_email(text) IS 
'Enmascara emails: abc***@domain.com';

COMMENT ON FUNCTION public.mask_phone(text) IS 
'Enmascara teléfonos: ***-***-1234';

COMMENT ON FUNCTION public.mask_cedula(text) IS 
'Enmascara cédulas: ***-***-123';

COMMENT ON TABLE public.data_access_audit IS 
'Auditoría de accesos a datos sensibles (ISO 27001 / GDPR)';
...
INSERT INTO public.security_audit_log (
  ...
  jsonb_build_object(
    'timestamp', now(),
    'tables_affected', ARRAY['miembros', 'seccionales', 'delivery_orders'],
    'policies_created', 5,
    'views_created', 3,
    'masking_functions_created', 4,
    'audit_table_created', true,
    'compliance_standards', ARRAY['ISO 27001', 'GDPR', 'Privacy by Design']
  )
);